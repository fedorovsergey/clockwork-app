...
- Исключение запросов из списка по URI (ignorePaths): кнопка «✕» в каждой строке таблицы (появляется при наведении), управление списком в настройках.
  - Для `.php` URI — query-строка отбрасывается, сохраняется только путь до `?`.
  - Для URI с `url=` — извлекается только значение параметра `url` (до `&`), фильтр работает по вхождению подстроки.
  - Для остальных URI — отсекается последний сегмент пути (после последнего `/`).
  - В настройках — поле ввода с тегами (Enter для добавления, ✕ для удаления), help-text с описанием форматов.
  - Фильтрация в `RequestsList.vue` (computed `requests`): regex (`/pattern/`), `url=` (вхождение подстроки), точное/префиксное совпадение.
